...
export const FIXTURE_SET: Array<{ input: PlanInput; edge: string; tag?: string }> = [
  {
    input: { chain: "ethereum", token: "0x6982508145454ce325ddbe47a25d4ec3d2311933", amount: 12_000_000_000 },
    edge: "1 · hero — PEPE 12B (≈$40K donation); large, liquid; exchange-driven red days in history",
  },
  ...
  {
    input: { chain: "ethereum", token: "0x95ad61b0a150d79219dcf64e1e6cc01f0b64c4ce", amount: 2_000_000_000 },
    edge: "7 · SHIB on ethereum — quote unsupported chain → labelled constant-product estimate",
  },
  {
    input: { chain: "base", token: "0x4ed4e862860bed51a9570b96d89af5e1b0efefed", amount: 5_000_000 },
    edge: "8 · DEGEN on base — thin organic demand ($1.6K/day in the spike): long calendar, liquidity cap",
  },
  ...
  {
    input: { chain: "ethereum", token: "0x4dfae3690b93c47470b03036a17b23c1be05127c", amount: 3_000 },
    edge: "11 · OG Pepe (2020, 37K total supply, 5 buyers/7d, no indicators) — thin: unsold remainder after 90 days, single-buyer dependence",
  },
  {
    input: { chain: "ethereum", token: "0x2de7b02ae3b1f11d51ca7b2495e9094874a064c0", amount: 100_000_000 },
    edge: "12 · SHIB2 — $0 DEX buys in 7 days → no organic demand, no calendar, numbers still shown",
  },
...
if (process.argv.includes("--rederive")) {
  ...
  for (const path of listFixtures()) {
    ...
    if (plan.hash !== f.plan.hash) {
      console.error(`✖ ${path}: hash changed ${f.plan.hash.slice(0, 12)} → ${plan.hash.slice(0, 12)} — the decision moved, re-seed live instead`);
      process.exit(1);
    }
    ...
    writeFixture(
      {
        ...f,
        plan: { ...plan, credits: f.plan.credits, calls: f.plan.calls, cachedCalls: f.plan.cachedCalls, ms: f.plan.ms, asOf: f.plan.asOf, provenance: f.plan.provenance, errors: f.plan.errors },
      },
      undefined,
      tag,
    );
...
for (const f of set) {
  ...
  const fixture: Fixture = {
    ...
    live: { calls: live.length, credits: plan.credits, ms: plan.ms },
  ...
  const out =
    plan.status === "ok" || plan.status === "thin"
      ? `${plan.days} tranches · dump $${Math.round(plan.dumpToday.costUsd ?? 0)} vs glide $${Math.round(plan.glidepath.costUsd ?? 0)} (${plan.glidepath.model})${plan.today.red ? " · RED today" : ""}`
      : `${plan.status.toUpperCase()} — ${plan.statusReason}`;
  console.log(
    `${(plan.resolved.symbol ?? f.input.token).padEnd(8)} ${f.input.chain.padEnd(9)} → ${path}  ${out} · ${plan.credits} cr / ${live.length} calls / ${(plan.ms / 1000).toFixed(1)}s · ${plan.hash.slice(0, 12)}${failed.length ? `  ⚠ failed: ${failed.map((c) => `${c.endpoint} (${c.error})`).join(", ")}` : ""}`,
  );
}
