...
const sh = (cmd: string) => { try { return execSync(cmd, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }); } catch (e) { return String((e as { stdout?: string }).stdout ?? "") + "\n__FAILED__"; } };

// files a stranger needs
for (const f of ["README.md", "DEMO.md", "ARCHITECTURE.md", "LICENSE", "docs/SCORING.md", "docs/DX-REPORT.md", ".github/workflows/ci.yml", "package.json", ".gitignore"]) add(`file ${f}`, existsSync(f));
...
add("docs/screenshots has 01..05.png", ["01", "02", "03", "04", "05"].every((n) => shots.some((s) => s.startsWith(n))), shots.join(", "));

// secrets and kitchen leaks
const gi = read(".gitignore");
for (const p of [".env", ".cache/", "node_modules/", ".next/", ".vercel"]) add(`.gitignore covers ${p}`, gi.split("\n").some((l) => l.trim() === p || l.trim() === p.replace(/\/$/, "")));
