...
export type Plan = {
  ...
  organic: {
    totalBuy7dUsd: number | null; proBuy7dUsd: number | null; organicBuy7dUsd: number | null; organicDailyUsd: number | null;
    organicShare: number | null; organicBuyers: number | null; proBuyers: number | null; proLabels: string[]; top1Share: number | null; top10Share: number | null;
  };
...
export const THETA_SM_SHARE = 0.10;
...
export function sizeTranches(amount: number, trancheTokens: number, price: number, now: number, todayRed: { red: boolean; reason: string | null }): Pick<Plan, "tranches" | "days" | "truncated" | "remainderTokens" | "remainderPct"> {
...
export function planHash(p: Plan): string {
  ...
  const body = {
    chain: p.input.chain, address: p.resolved.address, amount: p.input.amount, status: p.status,
    priceE12: p.price.usd == null ? null : Math.round(p.price.usd * 1e12),
    organicDailyUsdCents: cents(p.organic.organicDailyUsd), k: p.risk.k,
    ...
    truncated: p.truncated, remainderTokens: round(p.remainderTokens, 6),
    dumpCents: cents(p.dumpToday.costUsd), glideCents: cents(p.glidepath.costUsd), models: [p.dumpToday.model, p.glidepath.model],
  };
...
export function computePlan(facts: Facts, input: PlanInput, resolved: Plan["resolved"], now: number, searchPrice?: number | null): Plan {
  ...
  let priceUsd: number | null = null, priceSource = "unavailable";
  if (facts.marketCapUsd && facts.circulatingSupply && facts.circulatingSupply > 0) { priceUsd = facts.marketCapUsd / facts.circulatingSupply; priceSource = "token-information market_cap_usd / circulating_supply"; }
  else if (facts.fdvUsd && facts.totalSupply && facts.totalSupply > 0) { priceUsd = facts.fdvUsd / facts.totalSupply; priceSource = "token-information fdv_usd / total_supply"; }
  ...
  let organicBuy7dUsd: number | null = null, organicShare: number | null = null;
  if (totalBuy7dUsd != null) {
    if (proBuy7dUsd == null) { organicBuy7dUsd = totalBuy7dUsd; organicShare = null; warnings.push("pro-buyer split unavailable (who-bought-sold failed) — organic = all DEX buys"); }
    else { organicBuy7dUsd = Math.max(0, totalBuy7dUsd - proBuy7dUsd); organicShare = totalBuy7dUsd > 0 ? organicBuy7dUsd / totalBuy7dUsd : null; }
  }
  ...
  // status
  let status: PlanStatus = "ok", statusReason: string | null = null;
  if (priceUsd == null) { status = "no-price"; statusReason = `no price for this token (token-information: ${errors["token-information"] ?? "no market cap / supply"})`; }
  else if (organicDailyUsd == null) { status = "no-organic-demand"; statusReason = `buy volume unavailable (${errors["token-information"] ?? "token-information returned no spot metrics"})`; }
  ...
  // tranches
  let trancheUsd: number | null = null, trancheCapReason: Plan["trancheCapReason"] = null;
  ...
  const plan: Plan = {
    input, resolved, status, statusReason,
    price: { usd: priceUsd, source: priceSource }, positionUsd,
    ...
    quotes: null, warnings, errors, now, computedAt: new Date(now).toISOString(), hash: "",
  };
