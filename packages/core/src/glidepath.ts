...
export async function glidepath(client: NansenClient, input: PlanInput, opts: GlidepathOptions = {}): Promise<PlanResult> {
  ...
  const finish = (plan: Plan): PlanResult => {
    ...
    return {
      ...plan, provenance,
      credits: provenance.reduce((n, c) => n + c.credits, 0), calls: provenance.length, cachedCalls: provenance.filter((c) => c.cached).length,
      ms: Date.now() - t0, asOf: client instanceof CachedNansenClient ? client.oldestHit ?? null : null,
    };
  };
  const empty = (status: Plan["status"], reason: string, resolved: Plan["resolved"]): Plan => {
    const p: Plan = {
      input: { chain, token: input.token, amount }, resolved, status, statusReason: reason,
      price: { usd: null, source: "unavailable" }, positionUsd: null,
      organic: { totalBuy7dUsd: null, proBuy7dUsd: null, organicBuy7dUsd: null, organicDailyUsd: null, organicShare: null, organicBuyers: null, proBuyers: null, proLabels: [], top1Share: null, top10Share: null },
      ...
      savingsUsd: null, quotes: null, warnings: [], errors: {}, now, computedAt: new Date(now).toISOString(), hash: "",
    };
    p.hash = planHash(p);
    return p;
  };

  if (!isChain(chain)) return finish(empty("not-found", `unknown chain "${input.chain}" — Nansen TGM chains: ${TGM_CHAINS.join(", ")}`, { address: "", symbol: input.token, name: input.token, viaSearch: false, logo: null }));
  if (!(amount > 0) || !Number.isFinite(amount)) return finish(empty("not-found", "amount must be a positive number of tokens", { address: "", symbol: input.token, name: input.token, viaSearch: false, logo: null }));

  let res;
  try { res = await resolveToken(client, chain, input.token); }
  catch (e) { return finish(empty("not-found", `search/general failed: ${(e as Error).message.slice(0, 120)}`, { address: "", symbol: input.token, name: input.token, viaSearch: true, logo: null })); }
  if (!res.ok) {
  ...
  if (plan.status !== "no-price" && plan.status !== "no-organic-demand" && quoteSupported(chain) && opts.quotes !== false) {
    ...
    const legs = [{ label: "one-tranche", tokens: one?.tokens ?? 0 }, { label: "whole-bag", tokens: amount }];
