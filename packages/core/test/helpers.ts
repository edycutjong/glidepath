...
export function fakeFetch(routes: Route, log: Array<{ endpoint: string; body: Record<string, unknown>; method: string }> = []): typeof fetch {
  return async (url, init) => {
    ...
    return new Response(JSON.stringify(out), { status: 200, headers: { "content-type": "application/json", "x-nansen-credits-used": String(creditsFor(endpoint)), "x-nansen-credits-remaining": "60000" } });
  };
}
...
export function pepeFacts(over: Partial<Facts> = {}): Facts {
  return {
    chain: "ethereum", address: PEPE, symbol: "PEPE", name: "Pepe", logo: null, deployedAt: "2023-04-14 14:51:35",
    marketCapUsd: 1_404_580_000, circulatingSupply: 420_690_000_000_000, fdvUsd: 1_404_580_000, totalSupply: 420_690_000_000_000,
    liquidityUsd: 13_790_405, totalHolders: 400_472,
    totalBuy7dUsd: 3_235_907, uniqueBuyers7d: 528,
    proBuy7dUsd: 24_893, proBuyers: 1, proPages: 1, proLabels: ["nftsindubai.eth"],
    organicPage1Usd: 1_657_674, organicPage1Rows: 407, organicTop10Usd: 1_340_000, organicTop1Usd: 349_588,
    smNet1dUsd: 5, exNet1dUsd: -1_724_177, whaleNet1dUsd: 0, smWallets1d: 21, smNet7dUsd: 29_948, exNet7dUsd: -1_895_861,
    history: [
    ...
    marketCapGroup: "largecap", isStablecoin: false,
    errors: {}, ...over,
  };
}
...
export function pepeRoutes(endpoint: string, body: Record<string, unknown>): unknown {
  ...
  if (endpoint === "search/general") {
    ...
    ], entities: [], total_results: 3 };
  }
  if (endpoint === "tgm/token-information") return { data: { name: "Pepe", symbol: "PEPE", contract_address: PEPE, logo: null,
    token_details: { token_deployment_date: "2023-04-14 14:51:35", market_cap_usd: 1_404_580_000, fdv_usd: 1_404_580_000, circulating_supply: 420_690_000_000_000, total_supply: 420_690_000_000_000 },
    spot_metrics: { volume_total_usd: 6_788_322, buy_volume_usd: 3_235_907, sell_volume_usd: 3_552_454, unique_buyers: 528, unique_sellers: 859, liquidity_usd: 13_790_405, total_holders: 400_472 } } };
  if (endpoint === "tgm/who-bought-sold") {
    const f = (body.filters ?? {}) as Record<string, unknown>;
    if (f.include_smart_money_labels) return { data: [{ address: "0xae12", address_label: "nftsindubai.eth", bought_volume_usd: 24_893, bought_token_volume: 7.4e9 }], pagination: { page: 1, per_page: 1000, is_last_page: true } };
    return { data: [{ address: "0x0c0e", address_label: "High Balance", bought_volume_usd: 349_588 }, { address: "0x1", address_label: "", bought_volume_usd: 341_000 }, { address: "0x2", address_label: "", bought_volume_usd: 223_192 }, ...Array.from({ length: 50 }, (_, i) => ({ address: `0x${i}`, address_label: "", bought_volume_usd: 14_000 }))], pagination: { page: 1, per_page: 1000, is_last_page: true } };
  }
  if (endpoint === "tgm/flow-intelligence") {
    const tf = body.timeframe;
    return { data: [tf === "1d" ? { smart_trader_net_flow_usd: 5, exchange_net_flow_usd: -1_724_177, whale_net_flow_usd: 0, smart_trader_wallet_count: 21 } : { smart_trader_net_flow_usd: 29_948, exchange_net_flow_usd: -1_895_861, whale_net_flow_usd: 0, smart_trader_wallet_count: 38 }], warnings: ["exchange_wallet_count is always 0"] };
  }
  if (endpoint === "tgm/flows") {
    const label = body.label;
    const day = (d: string, inflow: number, outflow: number, complete = true) => ({ date: `${d}T00:00:00Z`, bucket_end: `${d}T00:00:00Z`, is_complete: complete, price_usd: 3.4e-6, token_amount: 1e14, value_usd: 3.4e8, holders_count: 100, total_inflows_count: inflow, total_outflows_count: -outflow });
    const rows = label === "smart_money"
  ...
  if (endpoint === "tgm/indicators") return { token_address: body.token_address, chain, token_info: { market_cap_usd: 1.4e9, market_cap_group: "largecap", is_stablecoin: false },
    risk_indicators: [{ indicator_type: "btc-reflexivity", score: "high", signal: 1.45, signal_percentile: 73 }, { indicator_type: "liquidity-risk", score: "medium", signal: 0.0099, signal_percentile: 51 }],
  ...
  if (endpoint === "trade/quote") {
    ...
    if (String(body.from_token).startsWith("EPjF")) return { quotes: [{ aggregator: "okx", inAmount: "5000000", outAmount: "198075386149", priceImpactPct: "0.11", inUsdValue: "5", outUsdValue: "4.9677", toTokenPrice: "0.000002508", fromTokenDecimals: "6", toTokenDecimals: "5" }], success: true };
    const inUsd = (amt / 1e5) * 2.5e-6;
    return { quotes: [{ aggregator: "okx", inAmount: String(amt), outAmount: "1", priceImpactPct: "0.29", inUsdValue: String(inUsd), outUsdValue: String(inUsd * 0.99), fromTokenDecimals: "5", toTokenDecimals: "6" }], success: true };
  }
