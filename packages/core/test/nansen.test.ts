...
describe("whoBoughtPaged", () => {
  const page = (n: number, last: boolean) => ({
    data: Array.from({ length: 3 }, (_, i) => ({ address: `p${n}-${i}`, bought_volume_usd: 10 })),
    pagination: { page: n, per_page: 1000, is_last_page: last },
  });
  ...
  it("stops at is_last_page and reports the page count", async () => {
    const c = fakeClient((_e, b) => {
      const p = (b.pagination as { page: number }).page;
      return page(p, p === 3);
    });
