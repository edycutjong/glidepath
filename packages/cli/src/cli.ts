...
const consumed = new Set(["--chain", "--amount", "--csv", "--ics"].flatMap((n) => { const i = args.indexOf(n); return i >= 0 ? [i, i + 1] : []; }));
...
if (flag("--json")) { console.log(JSON.stringify(p, null, 2)); process.exit(0); }
const csvOut = opt("--csv"), icsOut = opt("--ics");
...
if (p.status === "not-found") {
  ...
  console.log(`price ${p.price.usd == null ? "—" : `$${p.price.usd.toPrecision(4)}`} ${D}(${p.price.source})${X} · liquidity ${usd(p.liquidityUsd)} · holders ${p.totalHolders?.toLocaleString("en-US") ?? "—"}`);
  const o = p.organic;
  console.log(`organic buys ${B}${usd(o.organicDailyUsd)}/day${X} ${D}= (${usd(o.totalBuy7dUsd)} DEX buys 7d − ${usd(o.proBuy7dUsd)} by Smart Money/Fund/Whale/Exchange/bot users [${o.proBuyers ?? "?"} wallets]) / 7 · organic share ${pct(o.organicShare)} · ${o.organicBuyers ?? "?"} organic buyers${X}`);
